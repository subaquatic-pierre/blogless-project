...
resource "aws_s3_bucket" "frontend_main" {
  bucket = var.domain_name
}

resource "aws_s3_bucket_website_configuration" "frontend_main" {
  bucket = aws_s3_bucket.frontend_main.bucket
...
resource "aws_s3_bucket_acl" "frontend_main" {
  bucket = aws_s3_bucket.frontend_main.id
  acl    = "public-read"
}
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Principal": {
              "AWS": "${aws_cloudfront_origin_access_identity.frontend_main.iam_arn}"
              },
            "Action": [
                "s3:GetObject"
            ],
